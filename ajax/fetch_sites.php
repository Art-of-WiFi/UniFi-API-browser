            ...
            try {
                ...
                if (!empty($sites_raw) && is_array($sites_raw)) {
                    foreach ($sites_raw as $site) {
                        $results['data'][] = [
                            'site_id'        => $site['id'] ?? $unknown_string,
                            'site_full_name' => $site['meta']['name'] ?? $site['meta']['desc'] ?? $site['name'] ?? $site['id'] ?? $unknown_string,
                        ];
                    }
                    ...
                    usort($results['data'], function ($a, $b) {
                        return strcasecmp($a['site_full_name'], $b['site_full_name']);
                    });
                }
                ...
                $results['state']   = 'error';
                $results['message'] = 'Official API connection error. Please check the URL and API key in config/config.php!';
